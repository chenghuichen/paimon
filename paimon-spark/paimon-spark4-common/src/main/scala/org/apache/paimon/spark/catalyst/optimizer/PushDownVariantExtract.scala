...
/**
 * Pushes down [[VariantGet]] extraction into Paimon's shredded Variant reading, enabling
 * Parquet column pruning for shredded Variant columns.
 *
 * When a Variant column is stored in shredded format (typed_value sub-columns per field),
 * and the query only accesses specific fields via variant_get / try_variant_get, this rule
 * replaces the full Variant read with a struct-typed read that only scans the needed
 ...
 * - All usages of the variant column in the projection are through [[VariantGet]]
 *   (no direct attribute reference)
 * - The path argument of [[VariantGet]] is a literal
 */
