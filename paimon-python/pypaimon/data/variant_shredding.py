...
# ---------------------------------------------------------------------------
# Sub-field projection helpers
# ---------------------------------------------------------------------------

def build_sub_field_column_expr(
    variant_col_name: str,
    field_path: List[str],
    schema: VariantSchema,
) -> Optional[object]:
    """Build a PyArrow dataset field expression for a VARIANT sub-field.

    Traverses the VariantSchema to verify the path is shredded, then returns
    a ``pyarrow.dataset.Expression`` that projects the typed sub-column.

    Args:
        variant_col_name: The top-level VARIANT column name (e.g. ``'payload'``).
        field_path:       Dot-split path components (e.g. ``['age']`` or
                          ``['address', 'street']``).
        schema:           Top-level VariantSchema for the VARIANT column.

    Returns:
        A ``ds.field(...)`` expression, or ``None`` if the path is not shredded.
    """
    import pyarrow.dataset as ds

    # Build the Arrow nested field path:
    #   payload → typed_value → age → typed_value
    #   payload → typed_value → address → typed_value → street → typed_value
    path_parts = [variant_col_name]
    current_schema = schema

    if current_schema.typed_idx < 0 or current_schema.object_fields is None:
        return None

    for component in field_path:
        if current_schema.object_schema_map is None:
            return None

        idx = current_schema.object_schema_map.get(component)
        if idx is None:
            return None

        path_parts.append('typed_value')
        path_parts.append(component)
        current_schema = current_schema.object_fields[idx].schema

    if current_schema.typed_idx < 0:
        return None   # leaf not shredded

    path_parts.append('typed_value')
    return ds.field(*path_parts)


def extract_sub_field_from_column(
    column: pa.Array,
    schema: VariantSchema,
    field_path: List[str],
) -> pa.Array:
    """Extract typed values for a sub-field path from a shredded VARIANT column.

    Unlike full assembly, this reads only the typed sub-column without
    reconstructing the full variant binary — saving memory on the critical path.

    Args:
        column:     A PyArrow Array whose type is a shredded VARIANT struct.
        schema:     Top-level VariantSchema for the column.
        field_path: Path components identifying the target sub-field.

    Returns:
        A PyArrow Array of the typed values, or an all-null array if the path
        is not available in this column.
    """
    current_col = column
    current_schema = schema
    ...
    for component in field_path:
        ...
        try:
            current_col = current_col.field('typed_value').field(component)
    ...
    try:
        return current_col.field('typed_value')
    except (KeyError, AttributeError):
        return pa.nulls(len(column))
...
def sub_field_output_type(
    obj_fields: List[ObjectField],
    path: List[str],
) -> Optional[pa.DataType]:
    """Return the Arrow type of the output column for a sub-field projection path.

    Navigates ``obj_fields`` by the dot-split ``path`` and returns the Arrow
    type of the ``typed_value`` at that leaf — matching what
    ``build_sub_field_column_expr`` / ``extract_sub_field_from_column`` produce.

    Returns ``None`` if the path is not found in the schema.
    """
    for f in obj_fields:
        if f.field_name == path[0]:
            if len(path) == 1:
                return _leaf_arrow_type_for_write(f.schema)
            else:
                if f.schema.object_fields is not None:
                    return sub_field_output_type(f.schema.object_fields, path[1:])
                return None
    return None
