...
Covers all three user-facing features described in the Python API documentation:
  ...
  - Sub-field Projection: reading individual typed sub-columns from shredded
                          Parquet without full variant reconstruction.
...
7.  Sub-field Projection     – extract_sub_field_from_column; FormatPyArrowReader
                               Phase-1 and Phase-2 integration.
8.  Write-path shredding     – parse_shredding_schema_option,
                               shredding_schema_to_arrow_type, decompose_variant.
9.  Full-chain via Paimon API – write then read through CatalogFactory / Schema /
                               write_arrow / to_arrow for both plain VARIANT,
                               shredded VARIANT, and sub-field projection.
"""
...
# ===========================================================================
# 7. Sub-field Projection (Phase 2)
# ===========================================================================

class TestExtractSubField(unittest.TestCase):

    def _make_column(self, age_values, name_values, meta):
        t = _object_shredded_type(('age', pa.int64()), ('name', pa.string()))
        rows = [
            {
                'metadata': meta,
                'value': None,
                'typed_value': {
                    'age': {'value': None, 'typed_value': av},
                    'name': {'value': None, 'typed_value': nv},
                },
            }
            for av, nv in zip(age_values, name_values)
        ]
        return pa.array(rows, type=t)

    def test_extract_int_field(self):
        meta = _make_metadata('age', 'name')
        col = self._make_column([10, 20, 30], ['a', 'b', 'c'], meta)
        schema = build_variant_schema(col.type)
        extracted = extract_sub_field_from_column(col, schema, ['age'])
        self.assertEqual(extracted.to_pylist(), [10, 20, 30])

    def test_extract_string_field(self):
        meta = _make_metadata('age', 'name')
        col = self._make_column([1, 2], ['Alice', 'Bob'], meta)
        schema = build_variant_schema(col.type)
        extracted = extract_sub_field_from_column(col, schema, ['name'])
        self.assertEqual(extracted.to_pylist(), ['Alice', 'Bob'])

    def test_extract_missing_path_returns_nulls(self):
        meta = _make_metadata('age', 'name')
        col = self._make_column([1], ['a'], meta)
        schema = build_variant_schema(col.type)
        extracted = extract_sub_field_from_column(col, schema, ['nonexistent'])
        self.assertEqual(len(extracted), 1)
        self.assertIsNone(extracted[0].as_py())

    def test_extract_with_some_null_values(self):
        meta = _make_metadata('age', 'name')
        t = _object_shredded_type(('age', pa.int64()), ('name', pa.string()))
        rows = [
            {'metadata': meta, 'value': None, 'typed_value': {
                'age': {'value': None, 'typed_value': 5},
                'name': {'value': None, 'typed_value': None},
            }},
            {'metadata': meta, 'value': None, 'typed_value': {
                'age': {'value': None, 'typed_value': None},
                'name': {'value': None, 'typed_value': 'Y'},
            }},
        ]
        col = pa.array(rows, type=t)
        schema = build_variant_schema(col.type)

        ages = extract_sub_field_from_column(col, schema, ['age'])
        self.assertEqual(ages[0].as_py(), 5)
        self.assertIsNone(ages[1].as_py())


class TestFormatPyArrowReaderVariant(unittest.TestCase):
    """Integration tests for FormatPyArrowReader Phase-1 (assembly) and Phase-2 (sub-field projection).

    These tests write shredded or plain VARIANT Parquet files to a local temp
    directory, then read them back through FormatPyArrowReader with LocalFileIO —
    exercising the actual Paimon read path without any mocking.

    Phase 1: The reader transparently reassembles shredded VARIANT columns into
             the standard struct<value, metadata> form.
    Phase 2: The reader extracts typed sub-field columns directly from the
             shredded Parquet layout, skipping overflow bytes and sibling fields.
             Verified by checking the result schema contains no binary fields.
    """
    ...
    def _write_shredded_parquet(self, filename, gvs, obj_fields, target_type):
        """Shred a list of GenericVariant (or None) and write to a temp Parquet file."""
        std_type = _variant_arrow_type()
        rows = [
            {'value': gv.value(), 'metadata': gv.metadata()} if gv is not None else None
            for gv in gvs
        ]
        std_col = pa.array(rows, type=std_type)
        shredded = shred_variant_column(std_col, obj_fields, target_type)
        table = pa.table({'v': shredded})
        path = os.path.join(self.tmpdir, filename)
        pq.write_table(table, path)
        return path

    def _write_plain_parquet(self, filename, gvs):
        """Write a plain VARIANT column (struct<value, metadata>) to a temp Parquet file."""
        col = GenericVariant.to_arrow_array(gvs)
        table = pa.table({'v': col})
        path = os.path.join(self.tmpdir, filename)
        pq.write_table(table, path)
        return path

    def _make_reader(self, path, read_fields, variant_sub_fields=None):
        ...
        return FormatPyArrowReader(
            ...
            variant_sub_fields=variant_sub_fields,
        )

    def test_phase1_assembles_shredded_column(self):
        """FormatPyArrowReader transparently reassembles a shredded VARIANT column."""
        obj_fields = parse_shredding_schema_option(
            _schema_json('v', [('age', 'BIGINT'), ('city', 'VARCHAR')])
        )['v']
        ...
        path = self._write_shredded_parquet('phase1.parquet', gvs, obj_fields, target_type)

        read_fields = [DataField(id=0, name='v', type=AtomicType('VARIANT'))]
        reader = self._make_reader(path, read_fields)
        batch = reader.read_arrow_batch()

        self.assertIsNotNone(batch)
        self.assertEqual(batch.num_rows, 2)
        # Column must be plain struct<value, metadata> after Phase-1 assembly.
        v_col = batch.column('v')
        self.assertEqual(v_col.type, _variant_arrow_type())

        gv0 = GenericVariant.from_arrow_struct(v_col[0].as_py())
        self.assertEqual(gv0.to_python(), {'age': 28, 'city': 'Beijing'})
        gv1 = GenericVariant.from_arrow_struct(v_col[1].as_py())
        self.assertEqual(gv1.to_python(), {'age': 35, 'city': 'Shanghai'})

    def test_phase1_null_row_preserved(self):
        """Null VARIANT rows survive Phase-1 assembly unchanged."""
        obj_fields = parse_shredding_schema_option(
            _schema_json('v', [('x', 'BIGINT')])
        )['v']
        target_type = shredding_schema_to_arrow_type(obj_fields)

        gvs = [None, GenericVariant.from_python({'x': 7})]
        path = self._write_shredded_parquet('phase1_null.parquet', gvs, obj_fields, target_type)
        ...
        v_col = batch.column('v')
        self.assertIsNone(v_col[0].as_py())
        gv = GenericVariant.from_arrow_struct(v_col[1].as_py())
        self.assertEqual(gv.to_python(), {'x': 7})

    def test_phase1_overflow_fields_roundtrip(self):
        """Fields not in the shredding schema survive Phase-1 assembly via overflow bytes."""
        obj_fields = parse_shredding_schema_option(
            _schema_json('v', [('age', 'BIGINT')])
        )['v']
        target_type = shredding_schema_to_arrow_type(obj_fields)

        # 'note' is not shredded → goes to overflow
        gvs = [GenericVariant.from_python({'age': 42, 'note': 'extra'})]
        path = self._write_shredded_parquet('phase1_overflow.parquet', gvs, obj_fields, target_type)

        read_fields = [DataField(id=0, name='v', type=AtomicType('VARIANT'))]
        reader = self._make_reader(path, read_fields)
        batch = reader.read_arrow_batch()

        gv = GenericVariant.from_arrow_struct(batch.column('v')[0].as_py())
        result = gv.to_python()
        self.assertEqual(result['age'], 42)
        self.assertEqual(result['note'], 'extra')

    def test_phase2_sub_field_projection(self):
        """FormatPyArrowReader with variant_sub_fields returns typed struct column.

        Verifies both correctness (values match) and IO efficiency (the result
        struct contains no binary overflow fields — only the requested typed columns).
        """
        obj_fields = parse_shredding_schema_option(
            _schema_json('v', [('age', 'BIGINT'), ('city', 'VARCHAR')])
        )['v']
        ...
        gvs = [
            GenericVariant.from_python({'age': 10, 'city': 'A'}),
            GenericVariant.from_python({'age': 20, 'city': 'B'}),
        ]
        path = self._write_shredded_parquet('phase2.parquet', gvs, obj_fields, target_type)

        # Request two sub-fields; exclude parent VARIANT column from read_fields.
        reader = self._make_reader(
            path, read_fields=[], variant_sub_fields={'v': ['age', 'city']}
        )
        batch = reader.read_arrow_batch()

        self.assertIsNotNone(batch)
        v_col = batch.column('v')
        self.assertTrue(pa.types.is_struct(v_col.type))

        # Values correctness
        self.assertEqual(v_col.field('age')[0].as_py(), 10)
        self.assertEqual(v_col.field('age')[1].as_py(), 20)
        self.assertEqual(v_col.field('city')[0].as_py(), 'A')
        self.assertEqual(v_col.field('city')[1].as_py(), 'B')

        # IO efficiency: no binary overflow columns should appear in the result struct.
        # If the reader inadvertently included the overflow 'value' bytes, it would
        # show up as a binary-typed field here.
        for i in range(v_col.type.num_fields):
            self.assertFalse(
                pa.types.is_binary(v_col.type.field(i).type),
                f"Field '{v_col.type.field(i).name}' is binary — overflow bytes leaked into "
                f"sub-field projection result"
            )

    def test_phase2_null_row_produces_null_sub_fields(self):
        """A null VARIANT row yields null entries in the projected sub-field columns."""
        obj_fields = parse_shredding_schema_option(
            _schema_json('v', [('score', 'BIGINT')])
        )['v']
        target_type = shredding_schema_to_arrow_type(obj_fields)

        gvs = [None, GenericVariant.from_python({'score': 99})]
        path = self._write_shredded_parquet('phase2_null.parquet', gvs, obj_fields, target_type)

        reader = self._make_reader(path, read_fields=[], variant_sub_fields={'v': ['score']})
        batch = reader.read_arrow_batch()

        v_col = batch.column('v')
        # Row 0 is a null VARIANT → sub-field must also be null
        self.assertIsNone(v_col.field('score')[0].as_py())
        # Row 1 has score=99
        self.assertEqual(v_col.field('score')[1].as_py(), 99)

    def test_phase2_non_shredded_column_raises(self):
        """Requesting sub-field projection on a plain VARIANT column raises RuntimeError."""
        from pypaimon.filesystem.local_file_io import LocalFileIO
        from pypaimon.read.reader.format_pyarrow_reader import FormatPyArrowReader

        gvs = [GenericVariant.from_python({'x': 1})]
        path = self._write_plain_parquet('phase2_plain_err.parquet', gvs)
        ...
        with self.assertRaises(RuntimeError) as ctx:
            FormatPyArrowReader(
                ...
                read_fields=[],
                push_down_predicate=None,
                variant_sub_fields={'v': ['x']},
            )
        self.assertIn('shredded', str(ctx.exception).lower())


# ===========================================================================
# 8. Write-path shredding
# ===========================================================================
...
class TestVariantPaimonTable(unittest.TestCase):
    ...
    def test_sub_field_projection(self):
        """with_variant_sub_fields returns typed columns; result contains no binary overflow."""
        shredding_json = _schema_json('payload', [('age', 'BIGINT'), ('city', 'VARCHAR')])
        schema = Schema.from_pyarrow_schema(
            self._pa_schema(),
            options={'variant.shreddingSchema': shredding_json},
        )
        self.catalog.create_table('default.sub_field_proj', schema, False)
        table = self.catalog.get_table('default.sub_field_proj')

        gvs = [
            GenericVariant.from_python({'age': 10, 'city': 'A', 'extra': True}),
            GenericVariant.from_python({'age': 20, 'city': 'B', 'extra': False}),
        ]
        write_builder = table.new_batch_write_builder()
        table_write = write_builder.new_write()
        table_commit = write_builder.new_commit()
        ...
        # Read with sub-field projection
        read_builder = table.new_read_builder()
        read_builder.with_variant_sub_fields({'payload': ['age', 'city']})
        splits = read_builder.new_scan().plan().splits()
        result = read_builder.new_read().to_arrow(splits)

        # 'payload' column excluded from read_type; only typed sub-field struct appears
        # combine_chunks() converts ChunkedArray → StructArray so .field() works
        payload_col = result.column('payload').combine_chunks()
        self.assertTrue(pa.types.is_struct(payload_col.type))

        # Correctness
        self.assertEqual(payload_col.field('age')[0].as_py(), 10)
        self.assertEqual(payload_col.field('age')[1].as_py(), 20)
        self.assertEqual(payload_col.field('city')[0].as_py(), 'A')
        self.assertEqual(payload_col.field('city')[1].as_py(), 'B')

        # IO efficiency: no binary overflow bytes should appear in the result struct
        for i in range(payload_col.type.num_fields):
            self.assertFalse(
                pa.types.is_binary(payload_col.type.field(i).type),
                f"Field '{payload_col.type.field(i).name}' is binary — overflow bytes leaked "
                f"into sub-field projection result"
            )
