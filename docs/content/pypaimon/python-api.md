...
**VARIANT Sub-field Projection (shredded layout only)**

When reading a table written with `variant.shreddingSchema`, each shredded sub-field is stored as a
separate Parquet column group. Use `with_variant_sub_fields()` to read only the specific typed
sub-columns you need, skipping the overflow `value` bytes and all sibling sub-fields:

```python
# Table was written with 'payload.age' and 'payload.city' shredded
read_builder = (
    table.new_read_builder()
    .with_variant_sub_fields({'payload': ['age', 'city']})
)
result = read_builder.new_read().to_arrow(read_builder.new_scan().plan().splits())
# result schema: id, ..., payload: struct<age: int64, city: string>
# 'payload' is replaced by a struct of the requested typed sub-fields
```

The original `payload` column is replaced by a struct column of the same name, whose fields
correspond to the requested paths with their native Arrow types. Only the requested `typed_value` sub-columns are read from Parquet, significantly reducing IO when a VARIANT
column is wide but only a few sub-fields are needed.

`with_variant_sub_fields()` raises `RuntimeError` if the target column is not stored in shredded
Parquet format (i.e., the table was not written with `variant.shreddingSchema`).
